...
authRoute.post("/login/:userType", ensureGuest, async (req, res) => {
...
authRoute.get("/:userType/dashboard", (req, res) => {
    ...
    console.log(`${userType} with ID ${req.session.userId} accessed the dashboard.`);
    res.render(`${userType}Dashboard`, { user: { email: req.session.userEmail || userType } });
});
